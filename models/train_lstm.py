"""Train the LSTM model and save its final test metrics.
...
def build_model(input_shape: tuple[int, int]) -> tf.keras.Model:
    model = Sequential([Input(shape=input_shape), LSTM(64, return_sequences=True), Dropout(0.2), LSTM(32), Dropout(0.2), Dense(16, activation="relu"), Dense(1)], name="scats_lstm")
...
def main() -> None:
    ...
    model_path = saved_models_dir() / "lstm_model.keras"
    ...
    metrics_file = update_metrics_file("LSTM", metrics)
    history_file = update_history_file("LSTM", history.history)
    predictions_file = update_predictions_file(
        "LSTM", data["test_meta"], actual_flow, predicted_flow
    )
