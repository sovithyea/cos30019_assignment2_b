"""Train the CNN model and save its final test metrics.
...
def build_model(input_shape: tuple[int, int]) -> tf.keras.Model:
    model = Sequential([Input(shape=input_shape), Conv1D(64, 3, activation="relu", padding="causal"), MaxPooling1D(pool_size=2), Dropout(0.2), Conv1D(32, 3, activation="relu", padding="causal"), Flatten(), Dense(32, activation="relu"), Dropout(0.2), Dense(1)], name="scats_cnn_1d")
...
def main() -> None:
    ...
    model_path = saved_models_dir() / "cnn_model.keras"
    ...
    metrics_file = update_metrics_file("CNN", metrics)
    history_file = update_history_file("CNN", history.history)
    predictions_file = update_predictions_file(
        "CNN", data["test_meta"], actual_flow, predicted_flow
    )
